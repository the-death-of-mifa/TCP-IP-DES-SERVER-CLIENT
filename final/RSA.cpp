...
void setKey(mpz_t n, mpz_t e, mpz_t d, int nbits, int ebits) // n是公钥，e是公钥指数，d是私钥指数，nbits是密钥长度，ebits是公钥指数长度
{
    if (nbits / 2 <= ebits)
    {
        ebits = nbits / 2;
...
void RSAencrypt(mpz_t result, const mpz_t m, const mpz_t e, const mpz_t n)
{
    mpz_powm(result, m, e, n);
}

void RSAdecrypt(mpz_t result, const mpz_t c, const mpz_t d, const mpz_t n)
{
