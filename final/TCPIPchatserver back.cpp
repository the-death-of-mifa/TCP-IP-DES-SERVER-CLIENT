    ...
    while ((bytes_received = recv(clients[index].socket, buffer, BUFFER_SIZE, 0)) > 0)
    {
        ...
        if (strncmp(buffer, register_cmd, strlen(register_cmd)) == 0)
        {
            ...
            if (sscanf(buffer + strlen(register_cmd), " %127s %127s", username, password) == 2)
            {
                ...
                if (reval == 0)
                {
                    const char *success_message = "[系统消息] 注册成功！\n";
                    send(clients[index].socket, success_message, strlen(success_message), 0);
                }
                else
                {
                    const char *success_message = "[系统消息] 注册失败！\n";
                    send(clients[index].socket, success_message, strlen(success_message), 0);
                }
                return;
            }
        ...
        if (sscanf(buffer, "LOGIN %127s %127s", username, password) == 2)
        {
            if (authenticate_user(username, password))
            {
                ...
                const char *success_message = "[系统消息] 登录成功！\n";
                send(clients[index].socket, success_message, strlen(success_message), 0);
            }
            else
            {
                const char *failure_message = "[系统消息] 登录失败，用户名或密码错误。\n";
                send(clients[index].socket, failure_message, strlen(failure_message), 0);
            }
        }
        else
        {
            const char *error_message = "[系统消息] 登录格式错误。\n";
            send(clients[index].socket, error_message, strlen(error_message), 0);
        }
